...
export default function Home() {
  ...
  return (
    <div className="min-h-screen bg-[#f7f3ee] text-[#26323a]">
      ...
      {profileOpen && <div className="fixed right-4 top-[74px] z-50 w-56 rounded-2xl border border-[#e5dbd0] bg-[#fffdf9] p-3 shadow-[0_15px_40px_rgba(61,48,35,0.14)] sm:right-7 lg:right-10"><div className="flex items-center gap-3 border-b border-[#efe8e0] px-2 pb-3"><Avatar className="h-10 w-10"><AvatarFallback className="bg-[#e97848] text-xs font-bold text-white">RK</AvatarFallback></Avatar><div><p className="text-xs font-bold text-[#3f484b]">Raka</p><p className="text-[10px] text-[#a19485]">Pelajar SMA/SMK</p></div></div><button className="mt-2 flex w-full items-center gap-2 rounded-xl px-2 py-2 text-left text-xs font-bold text-[#827568] hover:bg-[#f6eee5]" onClick={() => { setProfileOpen(false); toast.success("Profil siap disesuaikan") }}><Star size={14} /> Sesuaikan profil</button><button className="flex w-full items-center gap-2 rounded-xl px-2 py-2 text-left text-xs font-bold text-[#827568] hover:bg-[#f6eee5]" onClick={() => { setProfileOpen(false); toast.success("Pengaturan disimpan") }}><Target size={14} /> Pengaturan target</button></div>}
